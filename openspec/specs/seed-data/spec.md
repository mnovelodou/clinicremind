...
Defines the `flask seed` developer/demo command that idempotently installs a
fixed, deterministic sample dataset into the iteration-1 schema — a clinic,
doctors, logins, patients, a receptionist grant, and appointments across the
status lifecycle — guarded against ever running in production.
