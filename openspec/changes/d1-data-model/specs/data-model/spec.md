...
The system SHALL provide a `doctors` table representing calendar owners. Each
doctor SHALL belong to exactly one clinic, have a name and creation timestamp,
and MAY optionally link to a user via a nullable `user_id`. A doctor without a
linked user is valid.
...
integer primary key, `clinic_id`, name, a canonical E.164 `phone` (with country
code), a `phone_national` (national significant number, digits only, no country
code) used for search, email, free-text notes, and `created_at`/`updated_at`
timestamps. Patients SHALL NOT include any clinical or medical fields. The
`phone_national` column SHALL be indexed together with `clinic_id`; `name` SHALL
also be indexed with `clinic_id`.
...
- **THEN** it contains only contact fields (name, phone, email, notes) and no
  clinical/medical columns

#### Scenario: Searchable by national number without country code

- **WHEN** the patients table is created
- **THEN** indexes exist on `(clinic_id, phone_national)` and
  `(clinic_id, name)`, allowing a patient to be found by their local number
  without typing the country code

#### Scenario: Canonical and searchable phone both stored

- **WHEN** a patient is saved with phone `+525512345678`
- **THEN** `phone` stores `+525512345678` and `phone_national` stores
  `5512345678`
...
`0001_baseline`, that creates all seven tables, the `appointment_status` and
`member_role` enum types, foreign keys, and the required indexes. The migration
SHALL apply and roll back cleanly. The `migrations/env.py` SHALL import the
models so `db.metadata` is fully populated.

#### Scenario: Upgrade creates the full schema

- **WHEN** `alembic upgrade head` runs against an empty database at the baseline
  revision
- **THEN** all seven tables, both enum types, and all indexes are created without
  error
